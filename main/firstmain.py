...
def listen_for_age(robotIP, PORT=9559):
...
def suggest_books(age_group, robotIP, PORT=9559):
    tts = ALProxy("ALTextToSpeech", robotIP, PORT)
...
def main(robotIP, PORT=9559):
    try:
        ...
        age_group = listen_for_age(robotIP, PORT)
        if age_group:
            suggest_books(age_group, robotIP, PORT)
        else:
