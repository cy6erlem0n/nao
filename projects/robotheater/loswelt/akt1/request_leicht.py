...
class MyClass(GeneratedClass):
    ...
    def first_word(self):
        ...
        try:
            ...
            self.speech.say("\\rspd=30\\ \\vct=50\\ d... ")
            self.speech.say("\\rspd=30\\ \\vct=50\\ die ")
            time.sleep(1.0)
            self.motion.setAngles("HeadPitch", 0.2, 0.1)  # Лёгкий наклон вниз
            self.motion.setAngles("HeadPitch", 0.0, 0.1)  # Возврат головы в центр

            self.speech.say("\\rspd=30\\ \\vct=50\\ die St... ")
            self.speech.say("\\rspd=30\\ \\vct=50\\ die Stimm ")
            self.speech.say("\\rspd=30\\ \\vct=50\\ die Stimmen ")
            time.sleep(1.0)
